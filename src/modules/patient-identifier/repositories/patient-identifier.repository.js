...
const findById = async (id, include = {}) => {
  try {
    return await prisma.patient_identifier.findFirst({
      where: {
        id,
        deleted_at: null
      },
...
const findMany = async (filters = {}, skip = 0, take = 20, orderBy = { created_at: 'desc' }, include = {}) => {
  try {
    ...
    return await prisma.patient_identifier.findMany({
      where,
      skip,
      take,
      orderBy,
...
const count = async (filters = {}) => {
  try {
    ...
    return await prisma.patient_identifier.count({ where });
  } catch (error) {
    throw new HttpError('errors.database.unexpected', 500, [{ originalError: error.message }]);
  }
};
...
const create = async (data) => {
  try {
    return await prisma.patient_identifier.create({
      data
    });
  } catch (error) {
    if (error.code === 'P2002') {
...
const update = async (id, data) => {
  try {
    return await prisma.patient_identifier.update({
      where: { id },
      data
    });
  } catch (error) {
