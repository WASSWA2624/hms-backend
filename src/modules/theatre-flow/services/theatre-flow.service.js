...
const mapAnesthesiaRecord = (entry) => ({
  ...entry,
  display_id: resolvePublicIdentifier(entry),
  theatre_case_display_id: toPublicScalarIdentifier(entry?.theatre_case_display_id),
...
const finalizeAction = async ({ theatreCaseId, action, context = {}, details = {} }) => {
  await createAuditLog({
    action,
    resource: 'theatre_flow',
    resource_id: theatreCaseId,
    user_id: context.user_id || null,
    ip_address: context.ip_address || null,
    details,
  }).catch(() => {});

  return getTheatreFlowById(theatreCaseId, { include_timeline: true });
};
...
const startTheatreFlow = async (data, context = {}) => {
  const result = await prisma.$transaction(async (tx) => {
    ...
    const payload = {
      ...
      status: toUpper(data?.status) || 'SCHEDULED',
      workflow_stage: sanitize(data?.workflow_stage) || 'PRE_OP',
...
const updateStage = async (id, data, context = {}) => {
  const result = await prisma.$transaction(async (tx) => {
    ...
    const payload = {};
    if (data.workflow_stage !== undefined) payload.workflow_stage = sanitize(data.workflow_stage) || null;
