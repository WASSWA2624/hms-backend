...
const resolvePositionByIdentifier = async (tx, identifier, tenantId, facilityId = null) => {
  ...
  return tx.staff_position.findFirst({
    where: {
      deleted_at: null,
      tenant_id: tenantId,
      AND: [
...
const resolveOrCreatePosition = async (tx, { positionId, positionName, tenantId, facilityId }) => {
  ...
  const existingByName = await tx.staff_position.findFirst({
    where: {
      deleted_at: null,
      tenant_id: tenantId,
      name: normalizedName,
  ...
  return tx.staff_position.create({
    data: {
      tenant_id: tenantId,
      facility_id: facilityId,
      name: normalizedName,
...
const validateRecurringConflictTx = async ({
  ...
  const existing = await tx.provider_schedule.findMany({
    where: {
      deleted_at: null,
      tenant_id: tenantId,
...
const validateOverrideConflictsTx = async ({
  ...
  for (const override of overrides) {
    ...
    const existing = await tx.availability_slot.findMany({
      where: {
        deleted_at: null,
        override_date: {
          gte: dayStart,
...
const createSchedulesTx = async ({
  ...
  if (normalizedOverrides.length > 0) {
    const targetSchedules = createdSchedules.length > 0
      ? createdSchedules
      : await tx.provider_schedule.findMany({
          where: {
            deleted_at: null,
            tenant_id: tenantId,
            facility_id: facilityId ?? null,
          ...
          orderBy: [{ day_of_week: 'asc' }, { created_at: 'asc' }],
        });
    ...
    for (const override of normalizedOverrides) {
      ...
      await tx.availability_slot.create({
        data: {
...
const listDoctors = async (filters = {}, page = 1, limit = 20, sortBy = 'created_at', order = 'desc') => {
  ...
  if (filters.facility_id) {
    const facility = await resolveFacilityByIdentifier(prisma, filters.facility_id, tenantId);
    if (!facility) {
      return { doctors: [], pagination: buildEmptyPagination(page, limit) };
    }
...
const createDoctor = async (data, actorUserId, ipAddress) => {
  const tenant = await resolveTenantByIdentifier(prisma, data.tenant_id);
  if (!tenant) {
  ...
  if (data.facility_id !== undefined && data.facility_id !== null) {
    facility = await resolveFacilityByIdentifier(prisma, data.facility_id, tenant.id);
    if (!facility) {
...
const updateDoctor = async (id, data, actorUserId, ipAddress) => {
  ...
  const updated = await prisma.$transaction(async (tx) => {
    ...
    const existingStaffProfile = await tx.staff_profile.findFirst({
      where: {
        deleted_at: null,
        user_id: before.id,
      },
    ...
    if (data.recurring_schedules !== undefined) {
      const existingSchedules = await tx.provider_schedule.findMany({
        where: {
          deleted_at: null,
          tenant_id: before.tenant_id,
          facility_id: targetFacilityId || null,
      ...
      if (existingScheduleIds.length > 0) {
        await tx.availability_slot.updateMany({
          where: {
            deleted_at: null,
            schedule_id: { in: existingScheduleIds },
          },
          data: {
            deleted_at: new Date(),
          },
        });
        await tx.provider_schedule.updateMany({
          where: {
            deleted_at: null,
            id: { in: existingScheduleIds },
          },
          data: {
            deleted_at: new Date(),
          },
    ...
    return tx.user.findFirst({
      where: { id: before.id, deleted_at: null },
      include: DOCTOR_INCLUDE,
    });
  });
