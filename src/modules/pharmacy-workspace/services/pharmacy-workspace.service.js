...
const buildInventoryStockWhere = async (filters = {}, options = {}) => {
  const includeSearch = options.includeSearch !== false;
  const where = {};

  if (filters.facility_id) {
    where.facility_id = await resolveModelIdOrThrow({
  ...
  if (filters.inventory_item_id) {
    where.inventory_item_id = await resolveModelIdOrThrow({
      identifier: filters.inventory_item_id,
      model: 'inventory_item',
      where: { deleted_at: null },
      errorKey: 'errors.inventory_item.not_found',
    });
...
const getPharmacyWorkbench = async (filters, page, limit, sortBy, order) => {
  try {
    ...
    const pendingAttestations = Math.max(0, preparedAttestations - completedAttestations);
...
const getPharmacyOrderWorkflow = async (identifier) => {
  try {
    ...
    const orderRecord = await pharmacyWorkspaceRepository.findOrderById(
      orderId,
      PHARMACY_ORDER_WITH_RELATIONS_INCLUDE
    );
    if (!orderRecord) {
...
const prepareDispense = async (identifier, payload = {}, userId, userRole, ipAddress) => {
  try {
    const orderId = await resolveModelIdOrThrow({
      identifier,
      model: 'pharmacy_order',
    ...
    const mutation = await pharmacyWorkspaceRepository.withTransaction(async (tx) => {
      const order = await pharmacyWorkspaceRepository.txFindOrderById(
        tx,
        orderId,
        PHARMACY_ORDER_WITH_RELATIONS_INCLUDE
      );
...
const attestDispense = async (identifier, payload = {}, userId, userRole, ipAddress) => {
  try {
    const orderId = await resolveModelIdOrThrow({
      identifier,
      model: 'pharmacy_order',
    ...
    const mutation = await pharmacyWorkspaceRepository.withTransaction(async (tx) => {
      ...
      const resolvedFacilityId = await resolveModelIdOrThrow({
        identifier: payload.facility_id || order.patient?.facility_id || null,
        model: 'facility',
        where: { deleted_at: null },
        errorKey: 'errors.facility.not_found',
        allowNull: true,
...
const cancelPharmacyOrder = async (identifier, payload = {}, userId, _userRole, ipAddress) => {
  try {
    const orderId = await resolveModelIdOrThrow({
      identifier,
      model: 'pharmacy_order',
...
const returnDispense = async (identifier, payload = {}, userId, userRole, ipAddress) => {
  try {
    const orderId = await resolveModelIdOrThrow({
      identifier,
      model: 'pharmacy_order',
...
const getInventoryStock = async (filters, page, limit, sortBy, order) => {
  try {
    const skip = (page - 1) * limit;
    const orderBy = sortBy ? { [sortBy]: order } : { updated_at: 'desc' };

    const [where, summaryWhere] = await Promise.all([
      buildInventoryStockWhere(filters, { includeSearch: true }),
      buildInventoryStockWhere(filters, { includeSearch: false }),
    ]);

    const [records, total, totalRecords, lowStockCount] = await Promise.all([
      ...
      pharmacyWorkspaceRepository.countInventoryStocks(summaryWhere),
      pharmacyWorkspaceRepository.countInventoryStocks({
        ...summaryWhere,
        quantity: {
          lte: prisma.inventory_stock.fields.reorder_level,
        },
    ...
    return {
      summary: {
        total_stock_rows: totalRecords,
        low_stock_rows: lowStockCount,
      },
...
const adjustInventoryStock = async (payload = {}, userId, _userRole, ipAddress) => {
  try {
    const inventoryItemId = await resolveModelIdOrThrow({
      identifier: payload.inventory_item_id,
      model: 'inventory_item',
      where: { deleted_at: null },
      errorKey: 'errors.inventory_item.not_found',
    });

    const facilityId = await resolveModelIdOrThrow({
      identifier: payload.facility_id || null,
      model: 'facility',
      where: { deleted_at: null },
      errorKey: 'errors.facility.not_found',
      allowNull: true,
    ...
    const mutation = await pharmacyWorkspaceRepository.withTransaction(async (tx) => {
      ...
      return {
        stock: refreshedStock || { ...stock, ...updatedStock },
        movement,
      };
    });
