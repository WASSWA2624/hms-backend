...
const listClinicalTermFavorites = async (filters = {}, context = {}) => {
  ...
  return prisma.clinical_term_favorite.findMany({
    where,
    orderBy: [{ usage_count: 'desc' }, { last_used_at: 'desc' }, { created_at: 'desc' }],
  });
};
...
const createClinicalTermFavorite = async (payload = {}, context = {}) => {
  ...
  const existing = await prisma.clinical_term_favorite.findFirst({
    where: {
      tenant_id: tenantId,
  ...
  const favorite =
    existing ||
    (await prisma.clinical_term_favorite.create({
      data: {
        tenant_id: tenantId,
        facility_id: facilityId,
        owner_user_id: ownerUserId,
...
const deleteClinicalTermFavorite = async (id, context = {}) => {
  ...
  const favorite = await prisma.clinical_term_favorite.findFirst({
  ...
  await prisma.clinical_term_favorite.update({
