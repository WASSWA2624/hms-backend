...
describe('Drug Service', () => {
  ...
  describe('listDrugs', () => {
    it('should list drugs with pagination', async () => {
      ...
      const result = await drugService.listDrugs({}, 1, 20, 'created_at', 'desc', mockUserId, mockIpAddress);
    ...
    it('should handle search filters', async () => {
      ...
      await drugService.listDrugs({ search: 'Para' }, 1, 20, null, 'asc', mockUserId, mockIpAddress);
    ...
    it('should handle name filter', async () => {
      ...
      await drugService.listDrugs({ name: 'Paracetamol' }, 1, 20, null, 'asc', mockUserId, mockIpAddress);
    ...
    it('should calculate pagination correctly', async () => {
      ...
      const result = await drugService.listDrugs({}, 2, 20, null, 'asc', mockUserId, mockIpAddress);
    ...
    it('should handle repository errors', async () => {
      ...
      await expect(drugService.listDrugs({}, 1, 20, null, 'asc', mockUserId, mockIpAddress)).rejects.toThrow(HttpError);
    });
  });

  describe('getDrugById', () => {
    it('should get drug by id', async () => {
      const mockDrug = { id: '123', name: 'Paracetamol' };
      drugRepository.findById.mockResolvedValue(mockDrug);

      const result = await drugService.getDrugById('123', mockUserId, mockIpAddress);
    ...
    it('should throw HttpError if drug not found', async () => {
      ...
      await expect(
        drugService.getDrugById('nonexistent', mockUserId, mockIpAddress)
    ...
    it('should handle repository errors', async () => {
      ...
      await expect(drugService.getDrugById('123', mockUserId, mockIpAddress)).rejects.toThrow(HttpError);
    });
  });
  ...
  describe('createDrug', () => {
    it('should create drug and log audit', async () => {
      ...
      const result = await drugService.createDrug(mockData, mockUserId, mockIpAddress);

      expect(result).toEqual(mockDrug);
      expect(drugRepository.create).toHaveBeenCalledWith(mockData);
    ...
    it('should propagate HttpErrors from repository', async () => {
      ...
      await expect(
        drugService.createDrug({}, mockUserId, mockIpAddress)
      ).rejects.toThrow(HttpError);
    });
    ...
    it('should handle unexpected errors', async () => {
      ...
      await expect(drugService.createDrug({}, mockUserId, mockIpAddress)).rejects.toThrow(HttpError);
    });
  });

  describe('updateDrug', () => {
    it('should update drug and log audit', async () => {
      const mockBefore = { id: '123', name: 'Paracetamol' };
      ...
      const result = await drugService.updateDrug('123', { name: 'Paracetamol Updated' }, mockUserId, mockIpAddress);

      expect(result).toEqual(mockAfter);
      expect(drugRepository.update).toHaveBeenCalledWith('123', { name: 'Paracetamol Updated' });
      expect(createAuditLog).toHaveBeenCalledWith(expect.objectContaining({
        user_id: mockUserId,
        action: 'UPDATE',
        entity: 'drug',
    ...
    it('should throw HttpError if drug not found', async () => {
      ...
      await expect(
        drugService.updateDrug('nonexistent', {}, mockUserId, mockIpAddress)
      ).rejects.toThrow(HttpError);
      await expect(
        drugService.updateDrug('nonexistent', {}, mockUserId, mockIpAddress)
      ).rejects.toMatchObject({
    ...
    it('should propagate HttpErrors from repository', async () => {
      drugRepository.findById.mockResolvedValue({ id: '123' });
      const httpError = new HttpError('errors.database.unique_field', 409);
      drugRepository.update.mockRejectedValue(httpError);

      await expect(
        drugService.updateDrug('123', {}, mockUserId, mockIpAddress)
      ).rejects.toThrow(HttpError);
    });
  });
  ...
  describe('deleteDrug', () => {
    it('should soft delete drug and log audit', async () => {
      ...
      await drugService.deleteDrug('123', mockUserId, mockIpAddress);
    ...
    it('should throw HttpError if drug not found', async () => {
      ...
      await expect(
        drugService.deleteDrug('nonexistent', mockUserId, mockIpAddress)
      ).rejects.toThrow(HttpError);
      await expect(
        drugService.deleteDrug('nonexistent', mockUserId, mockIpAddress)
      ).rejects.toMatchObject({
    ...
    it('should propagate HttpErrors from repository', async () => {
      ...
      await expect(
        drugService.deleteDrug('123', mockUserId, mockIpAddress)
      ).rejects.toThrow(HttpError);
    });
  });
});
