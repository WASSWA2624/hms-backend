...
describe('Inventory Item Service', () => {
  ...
  describe('listInventoryItems', () => {
    it('should list inventory items with pagination', async () => {
      ...
      const result = await inventoryItemService.listInventoryItems({}, 1, 20, 'created_at', 'desc', mockUserId, mockIpAddress);
    ...
    it('should handle search filters', async () => {
      ...
      await inventoryItemService.listInventoryItems({ search: 'Glove' }, 1, 20, null, 'asc', mockUserId, mockIpAddress);
    ...
    it('should handle name filter', async () => {
      ...
      await inventoryItemService.listInventoryItems({ name: 'Surgical Gloves' }, 1, 20, null, 'asc', mockUserId, mockIpAddress);
    ...
    it('should handle category filter', async () => {
      ...
      await inventoryItemService.listInventoryItems({ category: 'MEDICATION' }, 1, 20, null, 'asc', mockUserId, mockIpAddress);
    ...
    it('should calculate pagination correctly', async () => {
      ...
      const result = await inventoryItemService.listInventoryItems({}, 2, 20, null, 'asc', mockUserId, mockIpAddress);
    ...
    it('should handle repository errors', async () => {
      ...
      await expect(inventoryItemService.listInventoryItems({}, 1, 20, null, 'asc', mockUserId, mockIpAddress)).rejects.toThrow(HttpError);
    });
  });
  ...
  describe('getInventoryItemById', () => {
    it('should get inventory item by id', async () => {
      ...
      const result = await inventoryItemService.getInventoryItemById('123', mockUserId, mockIpAddress);
    ...
    it('should throw HttpError if inventory item not found', async () => {
      ...
      await expect(
        inventoryItemService.getInventoryItemById('nonexistent', mockUserId, mockIpAddress)
    ...
    it('should handle repository errors', async () => {
      ...
      await expect(inventoryItemService.getInventoryItemById('123', mockUserId, mockIpAddress)).rejects.toThrow(HttpError);
    });
  });
  ...
  describe('createInventoryItem', () => {
    it('should create inventory item and log audit', async () => {
      ...
      const result = await inventoryItemService.createInventoryItem(mockData, mockUserId, mockIpAddress);

      expect(result).toEqual(mockItem);
      expect(inventoryItemRepository.create).toHaveBeenCalledWith(mockData);
    ...
    it('should handle repository errors', async () => {
      ...
      await expect(
        inventoryItemService.createInventoryItem({}, mockUserId, mockIpAddress)
      ).rejects.toThrow(HttpError);
    });
  ...
  describe('updateInventoryItem', () => {
    it('should update inventory item and log audit', async () => {
      const mockBefore = { id: '123', name: 'Old Name', category: 'SUPPLY' };
      ...
      const result = await inventoryItemService.updateInventoryItem('123', mockData, mockUserId, mockIpAddress);

      expect(result).toEqual(mockAfter);
      expect(inventoryItemRepository.findById).toHaveBeenCalledWith('123');
      expect(inventoryItemRepository.update).toHaveBeenCalledWith('123', mockData);
      expect(createAuditLog).toHaveBeenCalledWith({
        user_id: mockUserId,
        action: 'UPDATE',
        entity: 'inventory_item',
    ...
    it('should throw HttpError if inventory item not found', async () => {
      ...
      await expect(
        inventoryItemService.updateInventoryItem('nonexistent', {}, mockUserId, mockIpAddress)
      ).rejects.toThrow(HttpError);
      await expect(
        inventoryItemService.updateInventoryItem('nonexistent', {}, mockUserId, mockIpAddress)
      ).rejects.toMatchObject({
    ...
    it('should handle repository errors', async () => {
      inventoryItemRepository.findById.mockResolvedValue({ id: '123' });
      inventoryItemRepository.update.mockRejectedValue(new Error('DB Error'));

      await expect(
        inventoryItemService.updateInventoryItem('123', {}, mockUserId, mockIpAddress)
      ).rejects.toThrow(HttpError);
    });

    it('should not fail if audit log fails', async () => {
      const mockBefore = { id: '123', name: 'Old Name' };
      const mockAfter = { id: '123', name: 'New Name' };
      inventoryItemRepository.findById.mockResolvedValue(mockBefore);
      inventoryItemRepository.update.mockResolvedValue(mockAfter);
      createAuditLog.mockReturnValue(Promise.reject(new Error('Audit failed')));

      const result = await inventoryItemService.updateInventoryItem('123', { name: 'New Name' }, mockUserId, mockIpAddress);
  ...
  describe('deleteInventoryItem', () => {
    it('should delete inventory item and log audit', async () => {
      ...
      await inventoryItemService.deleteInventoryItem('123', mockUserId, mockIpAddress);
    ...
    it('should throw HttpError if inventory item not found', async () => {
      ...
      await expect(
        inventoryItemService.deleteInventoryItem('nonexistent', mockUserId, mockIpAddress)
      ).rejects.toThrow(HttpError);
      await expect(
        inventoryItemService.deleteInventoryItem('nonexistent', mockUserId, mockIpAddress)
      ).rejects.toMatchObject({
    ...
    it('should handle repository errors', async () => {
      ...
      await expect(
        inventoryItemService.deleteInventoryItem('123', mockUserId, mockIpAddress)
      ).rejects.toThrow(HttpError);
    });

    it('should not fail if audit log fails', async () => {
      const mockItem = { id: '123', name: 'Surgical Gloves' };
      inventoryItemRepository.findById.mockResolvedValue(mockItem);
      inventoryItemRepository.softDelete.mockResolvedValue(undefined);
      createAuditLog.mockReturnValue(Promise.reject(new Error('Audit failed')));
