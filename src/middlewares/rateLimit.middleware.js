...
const rateLimit = (options = {}) => {
  ...
  const windowMs = options.windowMs || config.windowMs || 15 * 60 * 1000; // 15 minutes default
  const max = options.max || config.max || 100; // 100 requests default
...
const defaultRateLimit = () => {
  ...
  const envLimits = limits[NODE_ENV] || limits.production;
  
  return rateLimit(envLimits);
};
